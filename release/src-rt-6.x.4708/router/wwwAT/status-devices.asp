	...
	<script type="text/javascript">
		...
		dg.populate = function()
		{
			...
			for (i = list.length - 1; i >= 0; --i) {
				...
				b = e.mac;
				if (e.mac.match(/^(..):(..):(..)/)) {
					b += '<br /><a class="btn btn-small" href="http://standards.ieee.org/cgi-bin/ouisearch?' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="OUI Search"><% translate("OUI"); %></a> ' +
					'<a class="btn btn-small" href="javascript:addStatic(' + i + ')" title="Static Lease..."><% translate("Static"); %></a> ' +
					'<a class="btn btn-small" href="javascript:addqoslimit(' + i + ')" title="QoS BW Limiter"><% translate("BW Limit"); %></a>';

					if (e.rssi != '') {
						b += ' <a class="btn btn-small" href="javascript:addWF(' + i + ')" title="Wireless Filter..."><% translate("Wifi Filter"); %></a>';
					}
				}
				else {
					b = '';
