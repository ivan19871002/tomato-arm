		...
		<script type="text/javascript">
			...
			for (i = 0; i < tabs.length; ++i) {
				...
				htmlOut += createFormFields([
					...
					{ title: 'Extra HMAC authorization (tls-auth)', name: 'vpn_'+t+'_hmac', type: 'select', options: [ [-1,'<% translate("Disabled"); %>'], [2,'<% translate("Bi-directional"); %>'], [0, 'Incoming (0)'], [1, 'Outgoing (1)'] ], value: eval( 'nvram.vpn_'+t+'_hmac' ) },
				...
				htmlOut += createFormFields([
					...
					{ title: '<% translate("Accept DNS configuration"); %>', name: 'vpn_'+t+'_adns', type: 'select', options: [[0,'<% translate("Disabled"); %>'],[1, 'Relaxed'],[2, 'Strict'],[3,'<% translate("Exclusive"); %>']], value: eval( 'nvram.vpn_'+t+'_adns' ) },
